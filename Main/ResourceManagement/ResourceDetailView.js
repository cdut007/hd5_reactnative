		...
		if(!this.props.item.hasDone){
			...
			switch(this.props.type){
				case 4:
					...
					break;
				case 5:
					displayArr.push({title1:'退库量:',content1:this.props.item.number,color1:'#e82628',title2:'核实量:',content2:this.props.item.warrantyLevel,input2:true})
					displayArr.push({title1:'单价(￥):',content1:this.props.item.price,title2:'总金额(￥):',content2:this.props.item.ckJe})
					displayArr.push({title1:'仓库:',content1:this.props.item.warehouse,input1:true,title2:'货位:',content2:this.props.item.location,input2:true})
					viewArr = this.returnViewArr(displayArr);
					break;
			}
			if(viewArr.length > 0){
							...
							<View style={{marginLeft:10,flex:1, flexDirection:'row',alignItems:'center'}}>
								<Text style={{flex:7,fontSize:14, color:'#1c1c1c'}}>{item.title1}</Text>
								{
									...
									<TextInput
										...
										onChangeText={(text) => onTextChanged1(text)}
										placeholder={'请输入'}
										placeholderTextColor={'#6d9ee1'}
										underlineColorAndroid={'transparent'} />
									:
